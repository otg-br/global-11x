...
Imbuement* Imbuements::getImbuement(uint16_t id)
{
	...
	if (it == imbues.end()) {
		console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::getImbuement] Imbuement " + std::to_string(id) + " not found");
		return nullptr;
	}
	return &it->second;
}
...
bool Imbuements::loadFromXml(bool /* reloading */) {
	...
	for (auto baseNode : doc.child("imbuements").children()) {
		...
		if (strcasecmp(baseNode.name(), "base") == 0) {
			...
			if (!id) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing id for base entry");
				continue;
			}
			bases.emplace_back(
		...
		// Category/Group
			...
			pugi::xml_attribute id = baseNode.attribute("id");
			if (!id) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing id for category entry");
				continue;
			}
		...
		// Imbuements
			...
			if (!base) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing imbuement base id");
				continue;
			}
			...
			if (groupBase == nullptr) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Group base '" + std::to_string(baseid) + "' not exist");
				continue;
			}
			...
			if (!res.second) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Duplicate imbuement of Base ID: '" + std::to_string(baseid) + "' ignored");
				continue;
			}
			...
			if (!iconBase) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing iconid for imbuement entry");
				continue;
			}
			...
			if (!categorybase) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing imbuement category");
				continue;
			}
			...
			if (category_p == nullptr) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Category not exist");
				continue;
			}
			...
			if (!nameBase) {
				console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing imbuement name");
				continue;
			}
			...
			for (auto childNode : baseNode.children()) {
				if (!(attr = childNode.attribute("key"))) {
					console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing key attribute in imbuement id " + std::to_string(runningid));
					continue;
				}
				...
				if (strcasecmp(type.c_str(), "item") == 0) {
					if (!(attr = childNode.attribute("value"))) {
						console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing item ID for imbuement name '" + imb.name + "'");
						continue;
					}
					...
					if (it2 != imb.items.end()) {
						console::print(CONSOLEMESSAGE_TYPE_WARNING, std::string("[Warning - Imbuements::loadFromXml] Duplicate item ") + childNode.attribute("value").value() + ", imbuement name:" + imb.name + " ignored");
						continue;
					}
					...
					if (!(attr = childNode.attribute("type"))) {
						console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing effect type for imbuement name " + imb.name);
						continue;
					}
					...
					if (strcasecmp(effecttype.c_str(), "skill") == 0) {
						if (!(attr = childNode.attribute("value"))) {
							console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing effect value for imbuement name " + imb.name);
							continue;
						}
						...
						if (tmpStrValue == "sword") {
							...
							console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Unknow skill name '" + tmpStrValue + "' in imbuement name " + imb.name);
							continue;
						}


						if (!(attr = childNode.attribute("bonus"))) {
							console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing skill bonus for imbuement name " + imb.name);
							continue;
						}
						...
						if (!(attr = childNode.attribute("combat"))) {
							console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing combat for imbuement name " + imb.name);
							continue;
						}

						CombatType_t combatType = getCombatType(attr.as_string());
						if (combatType == COMBAT_NONE) {
							console::print(CONSOLEMESSAGE_TYPE_WARNING, std::string("[Warning - Imbuements::loadFromXml] Unknown combat type for element") + attr.as_string());
							continue;
						}

						if (!(attr = childNode.attribute("value"))) {
							console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing damage reduction percentage for imbuement name " + imb.name);
							continue;
						}
						...
						if (!(attr = childNode.attribute("value"))) {
							console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing speed value for imbuement name " + imb.name);
							continue;
						}
						...
						if (!(attr = childNode.attribute("value"))) {
							console::print(CONSOLEMESSAGE_TYPE_WARNING, "[Warning - Imbuements::loadFromXml] Missing cap value for imbuement name " + imb.name);
							continue;
						}
