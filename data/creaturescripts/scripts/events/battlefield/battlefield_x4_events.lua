function onLogout(player)
	local info = Battlefield_x4:findPlayer(player)
	if info then
		Battlefield_x4:onLeave(player)
	end
	return true
end
...
function onPrepareDeath(player, killer)
	local info = Battlefield_x4:findPlayer(player)
	if info then
		Battlefield_x4:onDeath(player, killer)
	end
	return false
end
...
function onHealthChange(player, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin)
	...
	local thisInfo = Battlefield_x4:findPlayer(player)
	local otherInfo = Battlefield_x4:findPlayer(attacker)
