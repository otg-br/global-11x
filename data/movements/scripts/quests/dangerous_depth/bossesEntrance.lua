function onStepIn(creature, item, position, fromPosition, toPosition)
	...
	if item:getPosition() == WarzoneIV then -- Warzone IV
		if creature:getStorageValue(Storage.DangerousDepths.Bosses.theBaronFromBelow) > os.stime() then
	...
	if item:getPosition() == WarzoneV then -- Warzone V
		if creature:getStorageValue(Storage.DangerousDepths.Bosses.theCountOfTheCore) > os.stime() then
			creature:teleportTo(fromPosition)
			creature:say('You have to wait to challange this enemy again!', TALKTYPE_MONSTER_SAY)
		else
	...
	if item:getPosition() == WarzoneVI then -- Warzone VI
		if creature:getStorageValue(Storage.DangerousDepths.Bosses.theDukeOfTheDepths) > os.stime() then
			creature:teleportTo(fromPosition)
			creature:say('You have to wait to challange this enemy again!', TALKTYPE_MONSTER_SAY)
		else
