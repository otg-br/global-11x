function onStepIn(creature, item, position, fromPosition)
	...
	if item.actionid == 5516 then
		if player:getStorageValue(Storage.CultsOfTibia.Orcs.bossTimer) > os.stime() then
	...
	if item.actionid == 5515 then
		if player:getStorageValue(Storage.CultsOfTibia.finalBoss.bossTimer) > os.stime() then
			player:sendCancelMessage('You need to wait for 20 hours to face this boss again.')
			player:teleportTo(fromPosition)
			return false
		end
	...
	if item.actionid == 5514 then
		if player:getStorageValue(Storage.CultsOfTibia.Misguided.bossTimer) > os.stime() then
		player:sendCancelMessage('You need to wait for 20 hours to face this boss again.')
		player:teleportTo(fromPosition)
		return false
		end
	...
	if item.actionid == 5513 then
		if player:getStorageValue(Storage.CultsOfTibia.Humans.bossTimer) > os.stime() then
			player:sendCancelMessage('You need to wait for 20 hours to face this boss again.')
			player:teleportTo(fromPosition)
			return false
		end
	...
	if item.actionid == 5512 then
		if player:getStorageValue(Storage.CultsOfTibia.Barkless.bossTimer) > os.stime() then
		player:sendCancelMessage('You need to wait for 20 hours to face this boss again.')
		player:teleportTo(fromPosition)
		return false
		end
	...
	if item.actionid == 5511 then
		if player:getStorageValue(Storage.CultsOfTibia.Minotaurs.bossTimer) > os.stime() then
		player:sendCancelMessage('You need to wait for 20 hours to face this boss again.')
		player:teleportTo(fromPosition)
		return false
		end
