...
local function sumonarLavasIV(i)
	...
	if boss == true then
		...
		elseif i == 0 then
			for j, position in pairs(aggressiveLavasIV) do
				local lava = Game.createMonster("Aggressive Lava", position, true, true)
			end
...
local function sumonarFieryHearts()
	...
	if boss and contagemHearts == 0 then
		for _, position in pairs(fieryHearts) do
			local fieryHeart = Game.createMonster("Fiery Heart", position, true, true)
			if fieryHeart then fieryHeart:registerEvent("fieryHearts") end
		end
		local oldBossHealth = bossId:getHealth()
		local oldBossPosition = bossId:getPosition()
		bossId:remove()
		local newBoss = Game.createMonster("the fire empowered duke", oldBossPosition, false, true)
		if newBoss then
...
local function summonBeasts(i)
	...
	elseif i == 0 then
		for j, position in pairs(beastPositions) do
			local beast = Game.createMonster("Ember Beast", position, true, true)
		end
	end
end
...
local function startWarzoneIV()
	addEvent(function()
		local monstro = Game.createMonster("The Baron From Below", Position(33648, 32303, 15), true, true)
			if monstro then
...
local function startWarzoneV()
	addEvent(function()
		local monstro = Game.createMonster("The Count Of The Core", Position(33681, 32334, 15), true, true)
			if monstro then
...
local function startWarzoneVI()
	addEvent(function()
		local monstro = Game.createMonster("The Duke Of The Depths", Position(33712, 32303, 15), true, true)
