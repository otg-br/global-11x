...
function onGetFormulaValues(player, skill, attack, factor, tool)
	...
	local min, max = player:getSpellDamage(-(((skillTotal * 0.01) + 1) + (levelTotal)), -(((skillTotal * 0.03) + 6) + (levelTotal)))

	local skillSecond = skill * element
	local minb, maxb = 0, 0
	if skillSecond > 0 then
		minb, maxb = player:getSpellDamage(-(((skillSecond * 0.01) + 1) + (levelTotal)), -(((skillSecond * 0.03) + 6) + (levelTotal)))
	end
