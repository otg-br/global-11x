...
local function transferFactory(playerName, amount, fromGuildId, info)
    return function(toGuildId)
        if not toGuildId then
            ...
            local fromGuid = Guild(fromGuildId)
            local toGuild = Guild(toGuildId)
            if not fromGuid or not toGuild then

            else
                local fromBalance = fromGuid:getBalance()
                if fromBalance < amount then
                    ...
                    fromGuid:setBalance(fromBalance - amount)
