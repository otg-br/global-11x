...
local quests = {
	[1] = {stg = Storage.secretLibrary.SmallIslands.Questline, value = 3},
	[2] = {stg = Storage.secretLibrary.LiquidDeath.Questline, value = 7},
	[3] = {stg = Storage.secretLibrary.Asuras.Questline, value = 6},
	[4] = {stg = Storage.secretLibrary.FalconBastion.Questline, value = 2},
	[5] = {stg = Storage.secretLibrary.Darashia.Questline, value = 8},
	[6] = {stg = Storage.secretLibrary.MoTA.Questline, value = 7}	
}

local function startMission(pid, storage, value)
	local player = Player(pid)
	if player then
		if player:getStorageValue(Storage.secretLibrary.Questlog) < 1 then
			player:setStorageValue(Storage.secretLibrary.Questlog, 1)
		end
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == 2 then
		if msgcontains(msg, "museum") then
			npcHandler:say({
				"I have heard that it was recently planned to expand the Museum of Tibian Arts. In the course of these activities unexpected difficulties occurred.",
			}, cid)
			startMission(player:getId(), Storage.secretLibrary.MoTA.Questline, 1)
		elseif msgcontains(msg, "desert") then
			npcHandler:say({
				"There are rumors of a mysterious statue in the desert next to Darashia. Nobody really knows the meaning of it.",
			}, cid)
			startMission(player:getId(), Storage.secretLibrary.Darashia.Questline, 1)
		elseif msgcontains(msg, "fishmen") then
			...
			startMission(player:getId(), Storage.secretLibrary.LiquidDeath.Questline, 1)
		elseif msgcontains(msg, "order") then
			...
			startMission(player:getId(), Storage.secretLibrary.FalconBastion.Questline, 1)
		elseif msgcontains(msg, "asuri") then
			...
			startMission(player:getId(), Storage.secretLibrary.Asuras.Questline, 1)
		elseif msgcontains(msg, "isle") then
			npcHandler:say({
				"Talk to Captain Charles in Port Hope. He told me that he once ran ashore on a small island where he discovered a small ruin. The architecture was like nothing he had seen before.",
			}, cid)
			startMission(player:getId(), Storage.secretLibrary.SmallIslands.Questline, 1)
		end
	elseif msgcontains(msg, 'progress') and player:getStorageValue(Storage.secretLibrary.libraryPermission) < 1 then
		npcHandler:say({
				...
				"Or shall me {check} how much information we acquired?"}, cid)
		npcHandler.topic[cid] = 3
	elseif npcHandler.topic[cid] == 3 then
		if msgcontains(msg, 'check') then
			if isQuestDone(player:getId()) then
				npcHandler:say({'As I told you: To enter the veiled library, travel to the white raven monastery on the Isle of Kings and enter its main altar room. ...',
				...
				player:setStorageValue(Storage.secretLibrary.libraryPermission, 1)
				...
				npcHandler:say({
				"You're still searching for informations.",
			}, cid)
			end
		end
